...
module.exports = function (app) {
     ...
     * req : post데이터에 file명칭의 파일과 유저token, registlocation(안드,애플,웹), filetype: [60201 video, 60202 profile, 60203 profileback], lat(profile 일땐 생략), lon(profile 일땐 생략), comment(profile 일땐 생략), capturedate(profile 일땐 생략)을 담아서 전송
                    ...
                    conn.query(sql, function (err, rows) {
                        ...
                        if (rows[2][0]['@fileid'] > 0) {
                            if (req.body.filetype === "60201") {
                                conn.query("CALL videoadd(@userid,@fileid," + req.body.lat + "," + req.body.lon + ",'" + req.body.comment + "','" + req.body.capturedate + "')", function (err, rows) {
                                    if (err) {
                                        res.json(result);
                                        conn.close();
                                        throw err;
