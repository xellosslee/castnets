module.exports = app => {
  ...
  route.post("/sendcertsms", (req, res, next) => {
    ...
    try {
      ...
      if (Number(result2) + 3000 <= Date.now()) {
        // 해당 암호문이 만들어진지 3초 이상 경과하였다면 오류로 판단
  ...
  route.post("/smscert", (req, res, next) => {
    ...
    try {
      var sql =
        "CALL smscert(" + req.body.logid + "," + req.body.certnum + ");";
      conn.query(sql, (err, rows) => {
        if (err) {
          res.json(result);
          conn.close();
  ...
  route.get("/profile/:name", (req, res, next) => {
    var conn = require("../modules/mysql.js")();
    var default_profile = path.join(
      __dirname,
      "../resources/image/castnetslogo.png"
    );
    try {
  ...
  route.post("/usernamecheck", (req, res, next) => {
    ...
    try {
      var sql =
        "CALL usernamecheck('" + req.body.token + "','" + req.body.name + "');";
      conn.query(sql, (err, rows) => {
        if (err) {
          common.sendResult(res, conn, resultcode.Failed);
          throw err;
